// ====================================================================
// 專案名稱：極簡日曆儀表板 (最終結構與圖片佈局版)
// 修正重點：修復 TypeError，並在 renderPageContent 函式中插入圖片元素。
// ====================================================================
...
function simplifiedToTraditional(text) {
    ...
    const map = { 
        ...
        '買': '買', '賣': '賣', '發': '發', '設': '設', '壇': '壇', '飾': '飾', '餘': '餘', '療': '療', 
        '理': '理', '歸': '歸', '竈': '竈' 
    };
...
function getDynamicAuspiceHours(date) {
    ...
    // 通過 getDayInGanZhi 獲取干支，然後提取日地支 (最後一個字)
    const dayBranch = dayGanZhi.substring(1); 
    
    let startBranchName;

    // 1. 確定黃道起始時辰 (根據日地支查表)
    switch (dayBranch) {
        case '子': case '午': startBranchName = '申'; break; 
        case '丑': case '未': startBranchName = '戌'; break; 
        case '寅': case '申': startBranchName = '子'; break; 
        case '卯': case '酉': startBranchName = '寅'; break; 
        case '辰': case '戌': startBranchName = '辰'; break; 
        case '巳': case '亥': startBranchName = '午'; break; 
